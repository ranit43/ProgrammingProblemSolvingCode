...
string anSt;
int pLen, tChar, arr[M], mat[M2][M2], dp[M][M2];

int Solve(int id, int prv)
{
    //if(pLen==id) cout << id << endl;
    if(id==pLen) return 0;
    ...
    {
        ret = min(ret, Solve(id+1, i)+abs(arr[id] - mat[prv][i]));
    }
    return ret;
}

void SolvePath(int id, int prv)
{
    //cout << id << endl;
    if(id==pLen) return;
    int ret = Solve(id, prv);

    for(int i=0; i<tChar; i++)
    {
        int tp = Solve(id+1, i) + abs(arr[id] - mat[prv][i]);
...
//    output.open("mersenePrime.txt");
    int t, i, j, k, tests, q, ans;

    while(cin >> pLen >> tChar)
    {
        for(i=1; i<pLen; i++) cin >> arr[i];
