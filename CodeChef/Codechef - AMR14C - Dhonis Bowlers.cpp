...
* A Hope is a very good thing. And No Good thing ever Dies !!! *
 ...
 Problem Name : AMR14C - Dhonis Bowlers
 Time Limit   : .000s
 Description  :
============================================================================*/
#include<algorithm>
...
int main()
{
    ...
    while(tests--)
    {
        ...
        for(i=0; i<n ; i++)
        {
            sum += upper_bound(modArr, modArr+n, x-modArr[i] )-modArr;   //modArr[i] & other numbers which total is less than or equal to x
            sum -= upper_bound(modArr, modArr+n, (m-1)-modArr[i] )-modArr;  //modArr[i] & other number which total is greater than x & smaller than equal to m-1
            sum += upper_bound(modArr, modArr+n, (m+x)-modArr[i] )-modArr;  //modArr[i] & other number which total is greater than m & which value after mod is smaller than or equal to x
        }
