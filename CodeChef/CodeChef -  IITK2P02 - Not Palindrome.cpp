...
LL tN, tM, mod=1000000007LL;

int main()
{
    //ios_base::sync_with_stdio(0); cin.tie(0);
    LL i, j, k, tests, q, ans;
    sf("%lld", &tests);
    while(tests--)
    {
        sf("%lld %lld", &tN, &tM);
        ans=tM%mod;
        if(tN>1) ans = (ans*(tM-1))%mod;
        if(tN>2) //tM>=3
        {
            ans = (ans*bigmod(tM-2, tN-2, mod))%mod;
        }
        pf("%lld\n", ans);
    }
...
Combinatorics
So we will start from left to right and try creating valid strings of length N. We can fill first position in M ways (use any character in the alphabet set). Then for the second position, you can not use the previous character, so there are M - 1 choices. For third position, you can not use both first and second position characters, so there are M - 2 choices. For any position >= 3, you can see that there will be M - 2 choices.
